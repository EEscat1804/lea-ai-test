...
def _is_minor(dob_str: str) -> bool:
    ...
    try:
        ...
        age = 2026 - dob.year - ((5, 27) < (dob.month, dob.day))
        return age < 18
    except ValueError:
        return False
...
def determine_next_step(jurisdiction: str, answers: dict[str, Any]) -> dict[str, Any]:
    # 1. Core Principle: Prioritize and evaluate Tier 1 sequentially (Executed with zero deviation across all states)
    for step in TIER_1_FLOW:
        field = step["field"]
        if field not in answers:
            # Q23: Dynamically inject Relationship option filtering specifically for California (CA)
            ...
            return {"step": field, "prompt": step["prompt"], "schema": schema_overlay}

    # 2. Core Principle: Fire Jurisdiction Gate immediately upon completion of Tier 1
    # For IL (Illinois), force an external Handoff interception after collecting Tier 1
    ...
    # Q24: Minor Filing Path (Triggered by DOB evaluation)
    if _is_minor(answers.get("petitioner.dob", "")) and jurisdiction in {"CA", "NY", "TX", "FL"}:
        if "petitioner.minor_filing_path" not in answers:
            return {
                "step": "petitioner.minor_filing_path",
                "prompt": "You're under 18 — is there an adult who can file with you, or do you want to file alone?",
                "schema": {"type": "string"}
            }

    # Native demographics and court assistance (Fixed original routing bugs to accurately parse by state)
    ...
    if jurisdiction in {"CA", "NY", "TX", "FL"}:
        ...
        physical_fields = [
            ...
            ("respondent.distinguishing_marks", prefix + "Any tattoos, scars, or other marks the sheriff would notice?", {"type": "string"}),
        ]
    ...
    # Texas (TX) Specific Background History Overlay (Q48, Q49)
    if jurisdiction == "TX":
    ...
    # Abuse Incident Police Report Connection Interception (Q50)
    if answers.get("incidents[].police_called") is True and jurisdiction in {"CA", "NY", "TX", "FL"}:
        if "incidents[].police_report_number" not in answers:
            return {
    ...
    # Firearm Threat Connection Interception (Q64, Q65)
    if answers.get("firearm.respondent_has_access") is True:
        if "firearm.types[]" not in answers:
            return {"step": "firearm.types[]", "prompt": "Do you know what kind? A handgun, a rifle, more than one?", "schema": {"type": "array", "items": {"type": "string"}}}
    ...
    # 5. Q118: Terminal Support Gate (Interception threshold for support relief cases)
    requested_reliefs = answers.get("selected_reliefs_intents", [])
    if jurisdiction in {"CA", "FL", "TX"} and ("child_support" in requested_reliefs or "spousal_support" in requested_reliefs):
        if "petitioner.ssn" not in answers:
            return {
                "step": "petitioner.ssn",
                "prompt": "(For child or spousal support, the court needs your SSN. Encrypted, never shared.)",
                "schema": {"type": "string", "pattern": "^\\d{3}-\\d{2}-\\d{4}$"}
            }

    # All checks passed successfully, transition to the Petition rendering stage
